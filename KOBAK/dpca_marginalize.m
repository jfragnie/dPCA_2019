...
% mean zero
X = bsxfun(@minus, X, mean(X(:,:),2)); %still a multidimensional (4D)array

% all parameter combinations
params = 2:length(size(X));   % return [2,3,4] for firingRateAVG
...
alreadyProcessed = containers.Map;  %just create a map with key-value
for subs = 1:length(paramsubsets)  %length(param..) = 7
    indRest = paramsubsets{subs};  %index de notre pos dans l'array.
    indMarg = setdiff(params, indRest); %setdiff(A,B) for vectors A and B, returns the values in A that 
                                        %are not in B with no repetitions. C will be sorted.
                                        %i.e. indMarg = setdiff(params,paramsubsets{1}) = 3 4
                                        %bc paramsubsets{1} = 2
                                        %indMarg =
                                        %setdiff(params,paramsubsets{7}) = empty vector 
                                        %bc paramsubsets{7} contains 2 3 and 4.
    YY{subs} = nanmmean(X, indMarg);    %nanmean(X,VECDIM) finds the mean of the elements of X based on the
                                        %dimensions specified in the vector VECDIM.
    alreadyProcessed(num2str(sort(indMarg))) = subs;  %sort=ascending order
    
    % subtracting all marginalizations from the subsets
    paramsubsubsets = subsets(indRest); % can be only one cell of paramsubsets
    for s = 1:length(paramsubsubsets)-1 %can be 0 1 or 2
        indM = [paramsubsubsets{s} indMarg];
        YY{subs} = bsxfun(@plus, YY{subs}, -YY{alreadyProcessed(num2str(sort(indM)))});
    end
end

paramsubsets = subsets(params-1);  %{1 2 3 [1 2] [1 3] [2 3] [1 2 3]}
margNums = 1:length(YY);

% combining some marginalizations together (OPTIONAL)
if ~isempty(options.combinedParams)
    for i=1:length(options.combinedParams)  %4 in their case
        margsToAdd = [];
        for j=1:length(options.combinedParams{i}) %j=1-3
