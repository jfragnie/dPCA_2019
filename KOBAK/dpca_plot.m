...
numCompToShow = min(options.numCompToShow, size(W,2)); %20 generally
...
if ~isempty(options.whichMarg) && ...
   length(unique(options.whichMarg)) <= 4 && length(unique(options.whichMarg)) > 1
    ...
    % if there are more than 4 marginalizations
    
    if isempty(options.whichMarg)
        % if there is no info about marginalizations
        componentsToPlot = 1:12;
    else
        % if there is info about marginalizations, select first 3 in each
        uni = unique(options.whichMarg);  %uni=1 2 3 4 for four marg posible
        componentsToPlot = [];
        for u = 1:length(uni)
            componentsToPlot = [componentsToPlot find(options.whichMarg==uni(u), 2)];
            %give only the 2 first margs that correspond to 1,2,3 or 4.
            %componentsToPlot is 2*nb_of_margs by construction.
        end
    ...
    subplots = [2 3 4 6 7 8 10 11 12 14 15 16]; %first one is empty
                                                %5,9&13 are different plots
    %not our case
    if numCompToShow < 12  
...
%size(Zfull) = [8     3     2    59]; for first dim is the nb of componentsToPlot
...
for c = 1:length(componentsToPlot) %creating each subplot separetely
    cc = componentsToPlot(c);
    subplot(4, 4, subplots(c))

    if ~isempty(options.componentsSignif)
        %time-periods of significant classification for each
        %component. componentsSignif = dpca_signifComponents(accuracy, accuracyShuffle, whichMarg)
    ...
    if ~isempty(options.whichMarg)
        ...
        thisYlim = 12;%options.ylims;
        thisMarg = [];
    end
    dim = size(Xfull);
    cln = {c};
    for i=2:length(dim)  %2 3 4 
        cln{i} = ':';   
        %cln: 1x4 cell: first cell: c = 1:length(componentsToPlot) : 1-8
        %here. the three other cells are ':'.
    end
 ...
 %% put the axis
    %subplot are numbered by lines, i.e the first subplot is empty
...
if ~isempty(options.explainedVar)
    ...
    b = bar(options.explainedVar.margVar(:,1:numCompToShow)' , 'grouped', 'BarWidth', 2);
...
%     end
    ...
    if ~isempty(options.explainedVar.cumulativeDPCA)
        err = options.explainedVar.SD;
        errorbar(1:numCompToShow, options.explainedVar.cumulativeDPCA(1:numCompToShow), ...
                err(1:numCompToShow), '.-r', 'LineWidth', 1, 'MarkerSize', 15);
    end
    
    %M1 PMv and PMd area!!
    if ~isempty(options.explainedVar.cumM1Var)
        plot(1:numCompToShow, options.explainedVar.cumM1Var(1:numCompToShow), ...
             '.-','color',[0, 0.4470, 0.7410] ,'LineWidth', 1, 'MarkerSize', 15);
    end
    if ~isempty(options.explainedVar.cumPMvVar)
        plot(1:numCompToShow, options.explainedVar.cumPMvVar(1:numCompToShow), ...
             '.-','color',[0.8500, 0.3250, 0.0980] ,'LineWidth', 1, 'MarkerSize', 15);
    end
    if ~isempty(options.explainedVar.cumPMdVar)
        
        plot(1:numCompToShow, options.explainedVar.cumPMdVar(1:numCompToShow), ...
             '.-','color', [0.9290, 0.6940, 0.1250],'LineWidth', 1, 'MarkerSize', 15);
    end
    ...
    xlabel('Component')
    if ~isempty(options.explainedVar.cumulativeDPCA)
        legend({'PCA', 'dPCA', 'M1', 'PMv', 'PMd'}, 'Location', 'SouthEast');
    else
...
%% angles and correlations between components: bottom left
