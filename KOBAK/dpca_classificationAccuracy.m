...
timeComp = [];  %timeComp = 2 for my data.
if ~isempty(options.combinedParams)  
    for k = 1:length(options.combinedParams) %4 for their data
        if options.combinedParams{k}{1} == length(size(Xfull))-1 %Length(size(Xfull))-1 =3 for their data, 2 for us
          %options.combinedParams{k}{1}= 1 2 3 [1 2]. for 1 2 3 4
            timeComp = k;  %=3 for their, 2 for us
            break
        end
    end
else
...
if ~isempty(options.combinedParams)
    ...
    %options.numComps = number of comp to test in each marginalization.
    %default is 3.
else
...
% set decoding classes. Not interesting us: we have a decoding class.
...
% compute dPCA on the full data  %%apparently cannot compute it as poorly
% conditionned matrix. With their data?
[~, Vfull, ~] = dpca(Xfull, numCompsToUse, ...
    ...
    'order', 'no', ...
    'Cnoise', Cnoise);
%% works until in this loop.
for rep = 1:options.numRep  %%write down the repetitions on the command window.
    ...
    for marg = setdiff(unique(whichMarg), timeComp)  %J marg=1 with our data
        dd = find(whichMarg==marg);%Theirs [1 2 4 5 6 8 10 11 14 15 17 18]
        for d = 1:length(dd)  %12
            ...
            for t=1:dim(end) %time: 1:59
                ...
                %trainClassMeans = accumarray(options.decodingClasses{marg}, trainClasses, [], @mean);
                %J size(trainClassMeans) = either [1 14] [1 2] or [1 7]
                
                testValues = XtestComp(c{:});
                testValues = testValues(:);
                
                dist = bsxfun(@minus, testValues, trainClassMeans'); %J size(trainClassMeans') = [1 3]
                size(trainClassMeans')
                %J dist= [-1.8299   -1.8299   -1.8299]; size=[1 3];
                [~, classification] = min(abs(dist), [], 2); %J classification is a scalar 
                
                correctClassifications(marg, d, t, rep) = length(find(classification == options.decodingClasses{marg}(:)));
                %correctClassifications(marg, d, t, rep) = length(find(classification == options.decodingClasses{marg})); %rid of (:)
                prob = 1./abs(dist);
                %J size(prob)
                prob = bsxfun(@times, prob, 1./sum(prob,2));
                actual = zeros(size(prob));  %size for their code [14 14]
                actual(sub2ind(size(actual), (1:size(actual,1))', options.decodingClasses{marg}(:))) = 1;
                %actual(sub2ind(size(actual), (1:size(actual,1))', options.decodingClasses{marg})) = 1; %rid of (:)
