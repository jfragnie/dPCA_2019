...
for rep = 1:options.numRep
    ...
    repTic = tic;
    %Xtrial = firingRates for their code
    %Xtest: 28x3x59 double
    %XtrainFull: 4D matrix 28 3 59 63
    %[Xtest, XtrainFull] = dpca_getTestTrials(firingRates, trialNum, 'simultaneous', options.simultaneous);
...
%     CnoiseTrain = diag(sum(bsxfun(@times, SSnoiseSumOverT(:,:), 1./(numOfTrials(:,:)-1)),2));
    ...
    XtestMargs = dpca_marginalize(XtestCen, 'combinedParams', options.combinedParams, ...
                    'ifFlat', 'yes');
    %XtestMargs: 1x3 cells : each contains 28X177 doubles (59X3=177)
