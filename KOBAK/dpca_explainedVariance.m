...
M1  = Xfull(:,1,:); M1TotVar = sum(sum(M1.^2));
PMv = Xfull(:,2,:); PMvTotVar = sum(sum(PMv.^2));
PMd = Xfull(:,3,:); PMdTotVar = sum(sum(PMd.^2));
M1=squeeze(M1); PMv=squeeze(PMv); PMd=squeeze(PMd);
...
for i=1:size(W,2)
    ...
    %For each brain areas
    explVar.cumM1Var(i) = 100 - sum(sum((M1 - V(:,1:i)*Z_1(1:i,:)).^2)) / M1TotVar * 100;
    explVar.cumPMvVar(i) = 100 - sum(sum((PMv - V(:,1:i)*Z_2(1:i,:)).^2)) / PMvTotVar * 100;
    explVar.cumPMdVar(i) = 100 - sum(sum((PMd - V(:,1:i)*Z_3(1:i,:)).^2)) / PMdTotVar * 100;
    ttt = [explVar.cumM1Var(i); explVar.cumPMvVar(i); explVar.cumPMdVar(i)];
    explVar.SD(i) = std(ttt);
...
if max(abs(explVar.cumulativePCA-explVar.cumulativeDPCA)) < 1e-10
    ...
    %same for brain areas var which is also based on DPCA axis.
    explVar.cumM1Var = [];
    explVar.cumPMvVar = [];
    explVar.cumPMdVar = [];
end
...
if ~isempty(options.Cnoise) && ~isempty(options.numOfTrials)
    ...
    if (size(Xfull,3) == 59)       
        S = size(Xfull,1);
        T = size(Xfull,2);
        ttt = [(S*T-T) (T-1)]/(S*T-1) * explVar.totalVar_noise;
    %5D matrix with days.
    else
        S = size(Xfull,2);
        Q = size(Xfull,3);
        T = size(Xfull,4);
        ttt = [(S*T-T) (Q*T-T) (T-1) S*Q*T-S*T-Q*T+T]/(S*Q*T-1) * explVar.totalVar_noise;
    end
    explVar.totalMarginalizedVar_signal = explVar.totalMarginalizedVar - ttt;
