...
N=84;   %nb of electrodes
T=59;   %nb of time points
E=63;   %nb of trials by rep
...
FR2 = FR_allByEpoch; 
clear FR_allByEpoch;
load('C:/Users/jules/Downloads/Projet_Bach/HD_data/Rey2036ElectrodeInfo.mat');
data=H_elec(1:128);
[data_mod,Type] = processData(data);

%% 12.02.19
[indDel1,FR1_red, averageFR] = findSmallIndex(FR1);
[AVG1,AVG2,AVG3] = brainAreas(Type,averageFR);
[firingRates,AVG1,AVG2,AVG3] = dimRed(AVG1,AVG2,AVG3,indDel1,FR1_red);
%trialNum:
[n1,n2,~,~] = size(firingRates); %if 4D
trialNum = size(firingRates,4).*ones(n1,n2);
firingRatesAverage = bsxfun(@times, mean(firingRates,3), size(firingRates,3)./trialNum);
%firingRatesAverage = squeeze(firingRatesAverage);  %3D matrix
%trialNum = nanmean(firingRatesAverage,2); %averaged over time.
%Nope: it doesnt correspond to the # of trials
...
margNames = {'time/brain areas Interaction'};
