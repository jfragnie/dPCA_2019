...
%     if ~exist([file_path file_prefix '-spikes-metatags.mat'],'file')
    if ~doSplit
        ...
        try
           %%TO BE CHANGED if needed! @J
            % NEVNSx_all.NEV = artifact_removal(NEVNSx_all.NEV,10,0.0005,1);
        catch
            keyboard
        end
        saveNEVSpikesCervical(NEVNSx_all.NEV, file_path, [file_prefix '-spikes.nev'])
        ...
        if exist([file_path file_prefix '-spikes-s.nev'],'file')        
           NEV_sorted = openNEVCervical('read',[file_path file_prefix '-spikes-s.nev']);
           % NEV_sorted = openNEVCervical('read',['C:\Users\jules\Downloads\Projet_Bach\HD_data\Rey\20190212\BLACKROCK_WIRED\' 'ReTh_20190212Rey' '-spikes-s.nev']);
            ...
            for iFile = 1:length(MetaTags.NEVlist) 
                ...
                NEV = openNEVCervical('read', [file_path MetaTags.NEVlist{iFile}],'nosave');
