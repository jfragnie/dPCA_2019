...
%file_path: (data_dir in example_..) -> C:\Users\jules\Downloads\Projet_Bach\HD_data\Rey\20190212\BLACKROCK_WIRED\
%file_prefix: ReTh_20190212_Rey_RecordBrain
%status = mattProcessSpikesForSorting(file_path,file_prefix,doSplit)
%NEVNSx_all = cerebus2NEVNSx(file_path,file_prefix);
    ...
    if (length(NEVlist_nodigital_sorted)==length(NEVlist_nospikes) && ~isempty(NEVlist_nospikes))  || (length(NEVlist_nodigital)==length(NEVlist_nospikes) && ~isempty(NEVlist_nospikes))
       for iNEV = 1:length(NEVlist_nospikes)
            ...
            if length(NEVlist_nodigital_sorted) >=length(NEVlist_nodigital) %if we have sorted data use that, otherwise use the base files
                NEV= openNEVCervical('read', [filepath filesep NEVlist_nodigital_sorted(iNEV).name],'nosave');
            else
                NEV= openNEVCervical('read', [filepath filesep NEVlist_nodigital(iNEV).name],'nosave');
            end
        ...
        if length(NEVlist_sorted)==length(NEVlist)
            ...
            for iNEV = 1:length(NEVlist)
                NEVNSxstruct(iNEV).NEV = openNEVCervical('read', [filepath filesep NEVlist(iNEV).name],'nosave');
            end
        end
    end
    ...
    for iNS = 1:5        
        for iFile = 1:length(eval(['NS' num2str(iNS) 'list']))
            NEVNSxstruct(iFile).(['NS' num2str(iNS)]) = openNSxCervical('read', [filepath filesep eval(['NS' num2str(iNS) 'list(iFile).name'])],'precision','short');
