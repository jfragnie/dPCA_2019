function Spikes = findSpikesCervical(NSx, varargin)
% findSpikes
%
...
if ~isempty(filter)
    [b, a] = butter(2, filterCorner/15000);
    data_thresh = filtfilt(b, a, Data);
    
    [b, a] = butter(2, [250 5000]/15000);
...
if isfield(NSx, 'ElectrodesInfo');
    ...
    threshold = cast(threshold, class(data_thresh));
end

[row, col] = find((diff(bsxfun(@minus, data_thresh, threshold) > 0) < 0).');
clear threshold;
